...
5. **LLM constraint (G3):** only allowed families — Gemini Flash (free), Llama via Groq (free), local Llama 3.x 1B–3B, local Phi Mini 3.5+. **Anthropic/Claude disqualifies.** Prefer `LLM_PROVIDER=groq` for voice latency.
